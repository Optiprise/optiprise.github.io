...
(function () {
  // -- Config: kies CDN of eigen kopie in /lib --
  var MARKED_SRC   = "https://cdn.jsdelivr.net/npm/marked/marked.min.js";
  var PURIFY_SRC   = "https://cdn.jsdelivr.net/npm/dompurify@3.1.7/dist/purify.min.js";

  // Laad een script en roep callback als klaar
  ...
  // Converteer markdown in een document (huidige pagina of iframe-doc)
  function convertMarkdownIn(doc) {
    if (!doc || !doc.querySelectorAll) return;

    // Converteer alléén in het "Documentation"-paneel als dat bestaat
    ...
    scope.querySelectorAll(".convertlinks").forEach(function (el) {
      ...
      // Archi encodeert vaak entities (&#233;). textContent decodeert ze.
      ...
      // Archi zet vaak white-space:pre-wrap; dat wil je niet na render
      try { el.style.whiteSpace = "normal"; } catch(e) {}

      // Links netjes in nieuw tabje
  ...
  // Wanneer uitvoeren?
  function wireUp(doc) {
    // 1) direct (als de pagina/iframe er al is)
    convertMarkdownIn(doc);

    // 2) als een Bootstrap-tab met documentatie wordt getoond
    ...
    // 3) fallback: als inhoud dynamisch wijzigt (sommige exports)
  ...
  function initForCurrentDocument() {
    // Deze file wordt in alle element/view pagina's geladen door Archi's templates
    wireUp(document);
  }

  // In index.html staan iframes (bv. name="view", name="element").
  // Als je ditzelfde frame.js óók in index.html zou laden, pak dan die frames mee:
      ...
      document.querySelectorAll("iframe").forEach(function (ifr) {
        ifr.addEventListener("load", function () {
          try {
            ...
            if (idoc) wireUp(idoc);
          } catch (e) { /* cross-origin komt hier niet voor in Archi export */ }
        });
      });
    } catch (e) {}
  }

  // Start: laad libs en ga daarna pas converteren
  function start() {
    Promise.resolve()
      ...
      .then(function () {
        if (document.readyState === "loading") {
          document.addEventListener("DOMContentLoaded", function () {
            ...
            initForIFrames(); // als dit bestand ook in index.html wordt gebruikt
          });
        } else {
          initForCurrentDocument();
          initForIFrames();
      ...
      .catch(function (e) { console.warn("Markdown setup mislukt:", e); });
  }
